...
# 如何获取窗口标题：powershell ./screenshot.ps1  （不入参即可）
# 已知问题：如果windows处于锁屏状态，无法获取像素点数据。
# by NoRain
# 2024/5/24
...
$Callback = [WinAPI+EnumWindowsProc] {
    ...
    if ([WinAPI]::GetWindowText($hWnd, $title, $title.Capacity) -gt 0 -and $title.ToString() -ne "" -and !($blacklist | Where-Object { $title.ToString().Contains($_) })) {
        ...
        if ($param -ne "" -and ($title.ToString() -eq $param -or $title.ToString() -eq $param1)) {
            Get-Screenshot $hWnd
...
# 检查 EnumWindows 函数的返回值
if (-not $result) {
    # Write-Host "EnumWindows 函数调用失败"
}
